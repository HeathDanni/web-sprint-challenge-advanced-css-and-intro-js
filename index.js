...
(1) Name of the first artist (0th index) in the 
...
function get20s(array){
  let artist20 = [];
    for (let i = 0; i < array.length; i++) {
      if (Number(array[i].years.substr(0, 4)) >= 1900 && Number(array[i].years.substr(8, 4)) <= 2000) {
...
addArtist(artists, 12, "heather", 34, "Web Dev", "American", "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse feugiat sollicitudin metus eget sodales.");
  ...
  let activeArtists = [];
    for (let i = 0; i < array.length; i++) {
      if (array[i]["paintings"] > 100) {
        activeArtists.push(array[i].name);
      }
